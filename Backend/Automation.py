...
import webbrowser  # Redundant import
...
classes = ["zCubwf", "hgKEIc", "LTkOO sy7ric", "zOLcw",  "gsrt vk_bk FzvWSb YwPhnf", "pclqee",
           "tw-Data-text tw-text-small tw-ta", "IZ6rdc", "O5uR6d LTkOO", "vLzy6d", "webanswers-webanswers_table_webanswers-table",
            "dDoNo ikb4Bb gsrt", "sXLaOe", "LwKfKe", "vQF4g", "qv3Wpe", "kno-rdcsc", "SPZz6b"]
...
SystemChatBot = [{"role": "system", "content": f"Hello, I am {os.environ['Username']}. You're a content writer. You have to write content like letters, articles, blogs, emails, etc."}]
...
def Content(Topic):
    ...
    def ContentWriterAI(prompt):
        ...
        messages.append({"role":"assistant", "content": Answer})#Add the AI respond to message
        return Answer
...
async def TranslateAndExecute(commands: list[str]):
    ...
    funcs = [] # List to asinchronous task.

    for command in commands :

        if command.startswith("open "): # Handal "open" commands.
            ...
            fun = asyncio.to_thread(PlayYouTube, command.removeprefix("play ")) # Schedule YouTube play
            funcs.append(fun)
